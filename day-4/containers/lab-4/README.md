...
docker run -e ENVIRONMENT=lab alpine printenv
```
...
```
docker run -e ENVIRONMENT=lab -e LANG=C.UTF-16 openjdk:8-jre-alpine printenv
```
...
📋 Run a container loading `labs/docker/env/exercises.env` as an environment file.
...
cat labs/docker/env/exercises.env

# run a container loading that file as environment variables:
docker run --env-file labs/docker/env/exercises.env alpine printenv
```
...
htmlPath="${PWD}/labs/docker/env/html"
